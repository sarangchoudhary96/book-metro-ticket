...
function getPath(station, path, requiredStation, stationObj) {
  for (var i = 0; i < station.length; i++) {
    ...
    if (
      station[i].diversion &&
      (getPath(station[i].diversion1, path, requiredStation, stationObj) ||
        getPath(station[i].diversion2, path, requiredStation, stationObj))
    ) {
      return true;
    }
  }
...
function updateRouteThroughDiversion(
  station,
  middleStations,
  distance,
  sourceObj,
  ...
  if (!sourceObj && !destObj) {
    ...
    distance.push(
      parseFloat(pathOne[pathOne.length - 1].distance) >
        parseFloat(pathOne[0].distance)
        ? parseFloat(pathOne[pathOne.length - 1].distance) -
            parseFloat(pathOne[0].distance)
        : parseFloat(pathOne[0].distance) -
            parseFloat(pathOne[pathOne.length - 1].distance)
    ...
    distance.push(
      parseFloat(pathTwo[pathTwo.length - 1].distance) >
        parseFloat(pathTwo[0].distance)
        ? parseFloat(pathTwo[pathTwo.length - 1].distance) -
            parseFloat(pathTwo[0].distance)
        : parseFloat(pathTwo[0].distance) -
            parseFloat(pathTwo[pathTwo.length - 1].distance)
  ...
  if ((sourceObj && !destObj) || (!sourceObj && destObj)) {
    distance.length = 0;
    distance.push(
      parseFloat(middleStations[middleStations.length - 1].distance) >
        parseFloat(middleStations[0].distance)
        ? parseFloat(middleStations[middleStations.length - 1].distance) -
            parseFloat(middleStations[0].distance)
        : parseFloat(middleStations[0].distance) -
            parseFloat(middleStations[middleStations.length - 1].distance)
...
function getRouteInfo(
  ...
  for (const station of stationsList) {
    if (
      station.stationName == sourceStation ||
      station.stationName == destStation
    ) {
  ...
  middleStations =
    !diversion && parseFloat(destObj.distance) < parseFloat(sourceObj.distance)
      ? middleStations.reverse()
      : middleStations;
  ...
  return {
    route_info: {
      [sourceLine]: (!sourceObj &&
        !destObj &&
        middleStations.map((middleStation) => {
          return {
            source: middleStation[0].stationName,
            destination: middleStation[middleStation.length - 1].stationName,
            middle_stations: [...middleStation.slice(1, -1)].map(
        ...
        })) || [
        {
          ...
          middle_stations: [...middleStations.slice(1, -1)].map(
            (item) => item.stationName
          ),
        },
      ],
    ...
    distance:
      (!diversion &&
        ((parseFloat(destObj.distance) < parseFloat(sourceObj.distance) &&
          parseFloat(sourceObj.distance) - parseFloat(destObj.distance)) ||
          parseFloat(destObj.distance) - parseFloat(sourceObj.distance))) ||
      distance.reduce((partialSum, a) => partialSum + a, 0),
    totalInterchange,
  };
}

function getSmallestPath(
  arr,
  sourceStation,
  destStation,
  tempArr,
  ...
  for (var i = 0; i < arr.length; i++) {
    var distance = getRouteInfo(
      dbData[tempArr[tempArr.length - 1].line],
      tempArr[tempArr.length - 1].station,
      arr[i].station,
      null,
      true
    );
...
function getCommonStations(routeInfo, sourceStation, destStation) {
  ...
  for (var i = 0; i < routeInfo.length - 1; i++) {
    arrOneArr.push({
      station:
        commonStation[routeInfo[i] + "_" + routeInfo[i + 1]] ||
        commonStation[routeInfo[i + 1] + "_" + routeInfo[i]],
      line: routeInfo[i + 1],
    });
